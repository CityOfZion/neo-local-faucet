...
export default class AddressForm extends React.Component {
  ...
  request = async () => {
    ...
    try {
      const { status, data } = await get(
        `${this.props.config.faucetSite}/request/${this.state.address}`
      );
      if (status === 200) {
  ...
  generate = async () => {
    ...
    const { status, data } = await get(
      `${this.props.config.faucetSite}/generate`
    );
    if (status === 200) {
  ...
  render() {
    ...
    if (this.state.status === faucetStatuses[3]) {
      successMessage =
        this.state.wif !== ""
          ? `Congratulations! Your address is ${
              this.state.generatedAddress
            } and your WIF is ${this.state.wif}, look for the following tx: ${
              this.state.tx
            }. Happy developing!`
          : `Congratulations, look for the following tx: ${
              this.state.tx
            }. Happy developing!`;
    }
    return (
        ...
        >
          ...
          <div style={{ textAlign: "right" }}>
            ...
            this.state.status === faucetStatuses[4] ? (
              ...
              <Button type="primary" disabled style={{ marginRight: "8px" }}>
                Generate
              </Button>
            )}
            {this.state.status === faucetStatuses[0] ||
            this.state.status === faucetStatuses[4] ? (
              <Button type="primary" onClick={this.request}>
                Request
              </Button>
            ) : this.state.status === faucetStatuses[1] ? (
